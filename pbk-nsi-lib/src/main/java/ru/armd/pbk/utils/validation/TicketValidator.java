package ru.armd.pbk.utils.validation;

import org.springframework.beans.factory.annotation.Autowired;
import ru.armd.pbk.core.utils.validation.BaseValidator;
import ru.armd.pbk.dto.nsi.TicketDTO;
import ru.armd.pbk.mappers.nsi.TicketMapper;
import ru.armd.pbk.utils.ValidationUtilMessages;

import javax.validation.ConstraintValidator;
import javax.validation.ConstraintValidatorContext;
import java.util.HashMap;
import java.util.Map;

/**
 * Реализация аннотации проверки билета.
 */
public class TicketValidator
	extends BaseValidator
	implements ConstraintValidator<ITicketValidator, TicketDTO> {

   @Autowired
   private TicketMapper mapper;

   @Override
   public void initialize(ITicketValidator constraintAnnotation) {
   }

   @Override
   public boolean isValid(TicketDTO value, ConstraintValidatorContext context) {
	  boolean valid = true;
	  if (value != null) {
		 Map<String, Object> filter = new HashMap<String, Object>();
		 filter.put("headId", value.getHeadId());
		 filter.put("cod", value.getCode());
		 if (mapper.isExist(filter)) {
			addConstraintViolation(context, ValidationUtilMessages.INCORRECT_COD);
			valid = false;
		 }

	  }
	  return valid;
   }

}