...
/**
 * Репозиторий видеорегистраторов.
 */
@Repository
public class DvrRepository
	extends BaseDeviceOwnersHistoryRepository<Dvr> {

   public static final Logger LOGGER = Logger.getLogger(DvrRepository.class);
   ...
   @Autowired
   DvrRepository(DvrMapper mapper) {
	  super(NsiAuditType.NSI_DVR, mapper);
	  this.dvrMapper = mapper;
   }
   ...
   public List<ISelectItem> getSelectItemsForEmployee(BaseSelectListParams params) {
	  ...
	  try {
		 sList = dvrMapper.getSelectItemsForEmployee(params.getFilter());
		 sList.add(0, new SelectItem(null, "Не выбрано"));
	  } catch (Exception e) {
		 String message = "Не удалось получить список видеорегистраторов. Ошибка: " + e.getMessage();
   ...
   public Employee getEmployeeByDeviceId(Long dvrId) {
	  return employeeRepository.getEmployeeByDeviceId(null, null, null, dvrId);
   }
}
